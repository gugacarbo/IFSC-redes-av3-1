...
class ProtocolHandlerTest {
  ...
  @Mock private PeerDiscovery peerDiscovery;

  @Mock private LivenessMonitor livenessMonitor;

  @Mock private PendingRequestTracker pendingRequestTracker;
  ...
  void testLeaveMessageRouting() throws Exception {
    ChatMessage leaveMessage = new ChatMessage("user1", "", MessageType.LEAVE);
  ...
  void testPongMessageRouting() throws Exception {
    ChatMessage pongMessage = new ChatMessage("user1", "", MessageType.PONG);
  ...
  void testAckMessageRouting() throws Exception {
    ChatMessage ackMessage = new ChatMessage("user1", "", MessageType.ACK);
  ...
  void testMessageWithNullType() throws Exception {
    String json =
        "{\"date\":\"25/03/2026\",\"time\":\"10:00:00\",\"username\":\"user1\",\"message\":\"test\",\"type\":null,\"msgId\":\"uuid-123\"}";
    byte[] data = json.getBytes(StandardCharsets.UTF_8);
    DatagramPacket packet = new DatagramPacket(data, data.length, remoteAddress, 5000);
  ...
  void testMessageWithNullUsername() throws Exception {
    String json =
        "{\"date\":\"25/03/2026\",\"time\":\"10:00:00\",\"username\":null,\"message\":\"test\",\"type\":\"CHAT\",\"msgId\":\"uuid-123\"}";
    byte[] data = json.getBytes(StandardCharsets.UTF_8);
    DatagramPacket packet = new DatagramPacket(data, data.length, remoteAddress, 5000);
  ...
  void testMessageWithEmptyMsgId() throws Exception {
    String json =
        "{\"date\":\"25/03/2026\",\"time\":\"10:00:00\",\"username\":\"user1\",\"message\":\"test\",\"type\":\"CHAT\",\"msgId\":\"\"}";
    byte[] data = json.getBytes(StandardCharsets.UTF_8);
    DatagramPacket packet = new DatagramPacket(data, data.length, remoteAddress, 5000);
