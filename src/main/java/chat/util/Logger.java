...
public class Logger {
  ...
  static {
    try {
      ...
      java.util.logging.FileHandler fileHandler =
          new java.util.logging.FileHandler(logPattern, true);
          ...
          new java.util.logging.SimpleFormatter() {
            ...
            public String format(java.util.logging.LogRecord lr) {
              return String.format(
                  "[%s] [%s] %s%n",
                  LocalDate.now().format(dateFormatter), lr.getLevel().getName(), lr.getMessage());
            }
          });
  ...
  private static void cleanOldLogs(File logDir) {
    File[] oldLogs =
        logDir.listFiles((dir, name) -> name.startsWith("chat_") && name.endsWith(".log"));
    if (oldLogs != null) {
